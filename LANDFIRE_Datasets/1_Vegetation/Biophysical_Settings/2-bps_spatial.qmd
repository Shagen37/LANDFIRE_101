...
::: callout-tip
*Pro tip:* The attribute for map zone in the BpS attribute table creates duplicate rows in the table in cases where one model applies to multiple map zones. For users only interested in unique BpS, we recommend performing a spatial lookup based on the BpS_Model code. This will allow the user to visualize each unique BpS in an area of interest.
:::
...
::: callout-tip
*Pro tip:* While there are various versions of the BpS map available for download, the mapping of the BpS has not changed with a couple of exceptions:

  1. The mapping of rock, sparse, barren, and water has changed overtime to keep it consistent with the location of these types in other LANDFIRE products.
