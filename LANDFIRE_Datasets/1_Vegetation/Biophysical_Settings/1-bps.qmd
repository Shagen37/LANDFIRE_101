...
For more information on ecological systems in LANDFIRE, see the [LANDFIRE Vegetation Products Overview](https://shagen37.github.io/LANDFIRE_101/LANDFIRE_Datasets/1_Vegetation/1_lf-vegetation-data.html) section of this website.
...
The spatial data, state and transition models and descriptions that comprise LANDFIRE’s BpS product suite offer a dynamic perspective on comprehensive on ecosystem ecology and set the stage for understanding current, and potential future ecosystem trajectories.
...
Next, we create [BpS Spatial Data](https://www.landfire.gov/vegetation/bps), which maps the distribution of these biophysical settings across the landscape. Linking the BpS map to the modeled estimate of fire frequency and severity allows LANDFIRE to map historical fire regimes.
...
Additionally, the BpS models and fire regime attributes contribute to the creation of the [Fire Behavior and Fuels Modeling (FBFM) Ruleset](https://www.landfire.gov/fuel/fuel_rulesets_db), which guides the application of fire and vegetation management practices.
...
The BpS Code serves as a crucial link connecting the various datasets within the LANDFIRE Biophysical Settings (BpS) framework. This [interconnected framework](https://tnc.app.box.com/s/jyostw32akl86j3s9q3481tjmkus9w58) ensures that all aspects of biophysical settings are comprehensively analyzed and utilized, supporting effective ecological management and planning. 

# A note about map zones

Map zones are used in LANDFIRE to break up the landscape into geographic areas suitable for mapping and modeling vegetation. With advances in computing, map zones are no longer used for mapping vegetation, but they are the basic geographic summary unit for BpS. BpS were originally developed at the scape of a map zone, and BpS reference conditions represent the average condition for the extent of a BpS across an entire map zone.
