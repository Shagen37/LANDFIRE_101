...
EVH is generated separately for tree, shrub, and herbaceous cover lifeforms using training data and other geospatial layers. Only the EVH for the primary upper layer lifeform is included in each pixel. Decision tree relationships are used to generate lifeform specific height class layers, which are merged into a single composite EVH layer. For more information on how EVH was created, review the [Technical Documentation](https://pubs.usgs.gov/publication/ofr20231045) provided on the LANDFIRE program website.
...
- [Heeren et al., (2023)](https://www.mdpi.com/2571-6255/6/12/474) developed a model to **map wildfire structure defensibility**, using LANDFIRE Existing Vegetation Height (and other LANDFIRE) datasets. As delivered the dataset represents the average height of a lifeform. The authors did not need lifeform, so they wrangled the data into a continuous height dataset that did not have herbaceous, shrub or tree modifiers.
...
- Detecting [changes in vegetation height](https://ntrs.nasa.gov/api/citations/20230006331/downloads/2023Spring_ID_SouthernIdahoEco_TechPaper_FD_v2.pdf) due to grazing